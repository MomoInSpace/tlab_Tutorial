program comm_test
    ...
    !allocate(x_d(grid_handler_A%total_space, 3), stat = ierr(1))
    !if (ierr(1) /= 0) print *, "q(1, grid_handler%total_space), : Allocation request denied"

    ! allocate(q(grid_handler%total_space, 1), stat = ierr(1))
    ! if (ierr(1) /= 0) print *, "q(1, grid_handler%total_space), : Allocation request denied"

    call grid_handler_A%set_pointer_1D(x(:,1))!, x(:,1))
    call grid_handler_B%set_pointer_1D(x(:,2))!, x(:,2))
    call grid_handler_tmp%set_pointer_1D(x(:,3))!, x(:,3))
    ...
    grid_handler_A%grid_space_main = my_rank + 1
    grid_handler_B%grid_space_main = 99

    if (my_rank == 0 ) then
        do i = 1, size(grid_handler_A%grid_space_main)
            grid_handler_A%grid_space_main(i) = i-1
        end do
    end if 
